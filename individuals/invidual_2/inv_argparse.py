...
if _main_:
    ...
    if not os.getenv('FILE_NAME'):
        filename = "NoFileName.json"
    
    if 'STUDENT_FILENAME' in os.environ:
        filename = os.environ['STUDENT_FILENAME']
    ...
    parsers['command'].add_argument(
        ...
        "--version",
        action="version",
        version="version: " + (os.getenv('VERSION_RELEASE') if os.getenv('VERSION_RELEASE') else '0.0.0')
    )
