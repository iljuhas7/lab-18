...
def student_save(file_path:str) -> bool:
    ...
    return inv_json.save(file_path, list_students)


def student_load(file_path:str) -> list:
    global list_students
    
    list_students = inv_json.load(file_path)
...
if _main_:
    ...
    inv_command.regist_command('select', student_print_list_select, 'вывести список студентов, имеющих Успеваемость')
    inv_command.regist_command('list', student_print_list, 'вывести список студентов')
    inv_command.regist_command('save', student_save, 'Сохранить', inv_command.cl_or_cmd['cmd'])
    inv_command.regist_command('load', student_load, 'Загрузить', inv_command.cl_or_cmd['cmd'])
    inv_command.regist_command('exit', student_exit, 'Выход', inv_command.cl_or_cmd['cmd'])
    inv_command.regist_command('cl', student_terminal, 'Терминал', inv_command.cl_or_cmd['cl'])
    
    # add help
    inv_command.add_arg_command('add', 'name', 'n', True, 'Ф.И.О.', str)
    inv_command.add_arg_command('add', 'group', 'g', False, 'Номер группы', int)
    inv_command.add_arg_command('add', 'year', 'y', True, 'Успеваемость', int)
    
    inv_command.add_arg_command('select', 'period', 'p', True, 'Успеваемость', int)
